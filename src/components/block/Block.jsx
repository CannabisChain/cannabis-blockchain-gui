...
export default function Block() {
  ...
  const poolReward = mojo_to_cannabis(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_cannabis(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const cannabisFees = blockRecord.fees
    ? mojo_to_cannabis(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`http://explorer.cannabischain.net/search/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`http://explorer.cannabischain.net/search/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: cannabisFees ? `${cannabisFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Cannabis blockchain
            </Trans>
          </BlockTitle>
        }
        action={
