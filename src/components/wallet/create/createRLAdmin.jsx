...
import { AlertDialog } from '@cannabis/core';
import {
  Typography,
  Button,
  Box,
...
import { cannabis_to_mojo } from '../../../util/cannabis';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      cannabisper_input.value === '' ||
      Number(cannabisper_input.value) === 0 ||
      !Number(cannabisper_input.value) ||
      isNaN(Number(cannabisper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const cannabisper = cannabis_to_mojo(cannabisper_input.value);
    const cannabisper_value = Number.parseInt(Number(cannabisper));
    const userpubkey = userpubkey_input.value;
    const amount = cannabis_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = cannabis_to_mojo(fee_input.value);
