...
import { AlertDialog, Card, Flex } from '@cannabis/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const cannabisper_input = ip_parsed.limit;
    ...
    const cannabisper_value = Number.parseInt(Number(cannabisper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (cannabis per interval):{' '}
                  {mojo_to_cannabis_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = cannabis_to_mojo(fee_input.value);
