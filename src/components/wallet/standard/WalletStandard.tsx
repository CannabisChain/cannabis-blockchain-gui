...
import {
  ...
  Fee,
  Form,
  TextField as CannabisTextField,
  AlertDialog,
  CopyToClipboard,
  Flex,
  Card,
...
import { /* mojo_to_cannabis_string, */ cannabis_to_mojo } from '../../../util/cannabis';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_cannabis_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of cannabis in the blockchain at the current
      ...
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the amount of Cannabis that you can currently use to make
            transactions. It does not include pending farming rewards, pending
            incoming transactions, and Cannabis that you have just spent but is not
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send cannabis to coloured address. Please enter a cannabis
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    if (address.slice(0, 12) === 'cannabis_addr://') {
    ...
    const amountValue = Number.parseFloat(cannabis_to_mojo(amount));
    const feeValue = Number.parseFloat(cannabis_to_mojo(fee));
