...
import cannabisEnvironment from '../util/cannabisEnvironment';
import cannabisConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!cannabisEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    cannabisConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!cannabisConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !cannabisConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (cannabisConfig.manageDaemonLifetime()) {
        cannabisEnvironment.startCannabisDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Cannabis Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/CannabisChain/cannabis-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/CannabisChain/cannabis-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/CannabisChain/cannabis-blockchain/releases',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/CannabisChain/cannabis-blockchain/issues',
              );
            },
          },
          {
    ...
    if (process.platform === 'darwin') {
      // Cannabis Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Cannabis' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Cannabis Blockchain' }),
            click: () => {
              openAbout();
            },
          },
