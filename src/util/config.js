...
global.daemon_rpc_ws = `wss://${self_hostname}:54400`;
...
function loadConfig(net) {
  try {
    // check if CANSCOIN_ROOT is set. it overrides 'net'
    const config_root_dir =
      'CANSCOIN_ROOT' in process.env
        ? process.env.CANSCOIN_ROOT
        : path.join(os.homedir(), '.cannabis', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 54400);
