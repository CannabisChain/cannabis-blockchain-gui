...
const PY_FOLDER = '../cannabis/daemon';
...
const getCannabisVersion = () => {
  let version = null;
  const exePath = getExecutablePath('cannabis');
  // first see if we can get a cannabis exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or cannabis is on the path
    try {
      version = child_process
